...
function StorageAction() {
  ...
  {storage.images.map((image, index) => (
    <img key={index} src={image} alt={`Im/age ${index}`} style={{ width: '100px', height: '100px', objectFit: 'cover' }} />
             ...
             {/*  <td style={{ border: '1px solid #dddddd', textAlign: 'left', padding: '8px' }}>
                <button onClick={() => acceptStorage(storageR.id)} style={{ ...acceptButtonStyle, ...buttonStyle }}>Accept</button>
                <button onClick={() => rejectStorage(storageR.id)} style={{ ...rejectButtonStyle, ...buttonStyle }}>Reject</button>
              </td> */}
            </tr>
          ))}
          <tr style={{ backgroundColor: '#f2f2f2' }}>
